...
# Participants are faster in round relative to round, an effect that is stronger when including all data
...
# Look at past outcome effect and strategy
...
contextResults = as.data.frame(matrix(data=NA,nrow=nSub, ncol=3, dimnames=list(c(NULL), c("subID", "pocBeta", "pocPval"))))
...
for (s in 1:nSub) {
  ...
  subsContextMods[[s]] = glm(choice~pastOC1sc, data=sub, family="binomial", offset=pred)
  ...
  contextResults$pocPval[s] = tmp$coefficients[[8]]; # store poc pvalue
