...
par(pty="s")
plot(c(pgamRoundxCondition[1], pgamRoundxCondition[3]), ylim = c(.4,.6), type="l", axes = F, lwd = 5, col="goldenrod1", ylab="P(gamble)", xlab="Round")
lines(c(pgamRoundxCondition[2], pgamRoundxCondition[4]), lwd=5, col="tomato1")
axis(1, at=c(1,2), label=c("1", "2"), lwd=6, cex=1.5)
axis(2, lwd =6, cex=1.5)
abline(a=.5, b=0, col="darkgrey", lty="dashed", lwd=3)

legend("bottomright", legend=c("Act Natural", "Strategy"), lty=c("solid", "solid"),lwd = 3, bty="n", col=c("goldenrod1","tomato1"), cex=1)
dev.off()
```
...
# trending negative effect of strategy? p=.08 (this is SUMMED on top of strategy effect in model 3 trial level). Because its stacking on a positive effect (Esp after accounting for past outcome effect) and its likely a net positive effect.
...
# 18005 - these models with positive and negative shift (vs signed shift) are allowing asymmetries between the positive and negative shift effect 
