...
# participants rated, on average, that they were able to follow instructions about 75% of the time with a slightly higher rating reported for the strategy condition
...
conditionBeta = 0.11460
roundBeta = 0.04872
condRoundIntxnBeta = 0.04107 

rounds = c(-1,-1,1,1)
conditions = c(-1,1,-1,1)

pgamRoundxCondition = 1/(1+exp(-1*( (conditionBeta*conditions) + (roundBeta*rounds) + (condRoundIntxnBeta*conditions*rounds) )));

# natural, round 1 pgam = 0.4694755
# natural, round 2  pgam = 0.4732880
# strategy, round 1 pgam = 0.5062022
# strategy, round 2 pgam = 0.5509204
...
# we see similar effects but they are weaker in this model and no interaction betwen round and strategy
...
# positive trending interaction between past outcome and strategy but no main effects of outcome (trending at p=.07) or strategy
...
# similar results above with trending positive interaction between outcome and strategy
...
summary(model_posNegShift_pred3)  # 18007
...
summary(model_posNegShiftStrat_pred1) # 18023 (very similar to signedShift model)
...
# 20053 - these models with positive and negative shift (vs signed shift) are allowing asymmetries between the positive and negative shift effect 
...
# positive interaction between earnings and outcome and pastoutcome and expectations
# AIC = 19916
...
# negative effect of past outcome
# positive, trending interaction bewteen earnings and outcome
# AIC =  19913- this is lower than above when including trial in interaction
...
# AIC = 19894- lower than pred 3 model
# results are similar but now a negative effect of strategy
...
# negative past outcome effect
# effect of negative shift only
# trending interaction between past outcome and earnings 
# smaller AIC here
...
# positive effect of trial/expectations
# positive interaction between poc and earnings across rounds
# positive interaction between poc and strategy (this would mean the effect of past outcome is different in the two rounds)
# positive interaction bewteen positive shift and strategy
# trending interaction nbewteen trial and strategy
...
# the strategy effect does seem to change one timescale but not the other two and might even make another one more intense.
