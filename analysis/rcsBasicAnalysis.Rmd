...
#  STILL NEED TO PLOT THIS

# overall poc effect for
 # moderate = -0.188832
 # high = -0.188832 + 0.175040 = -0.013792
 # low  = -0.188832 + -0.196764 = -0.385596


# poc x strategy effect for
  # moderate (nat) =  -0.188832 - 0.376590 = -0.565422
  # moderate (strat) = -0.188832 + 0.376590 = 0.187758
  # high (nat) = 0.376590 + -0.193435 = .18
  # high (strat) = 
  # low =  0.376590 + -0.622534 = -.25
...
basemodel1_stratTemporalcontextERQmedSplit  = summary(pocShiftEarnExp_ERQreapXstratXpoc_highLowReapMedSplit_1var); # using model with ERQ x 3 timescales
pocBeta = basemodel1_stratTemporalcontextERQmedSplit$coefficients[1]
stratBeta =  basemodel1_stratTemporalcontextERQmedSplit$coefficients[2]
reapBeta =  basemodel1_stratTemporalcontextERQmedSplit$coefficients[3]
pocStratBeta = basemodel1_stratTemporalcontextERQmedSplit$coefficients[7]
pocReapBeta = basemodel1_stratTemporalcontextERQmedSplit$coefficients[8]
stratReapBeta = basemodel1_stratTemporalcontextERQmedSplit$coefficients[9]
pocStratReapBeta = basemodel1_stratTemporalcontextERQmedSplit$coefficients[13]


# plot results of poc x strategy x reappraisal at different levels of reappraisal -> low, high
# each figure will have two lines for strategy and reappraisal
pocVals = rep(seq(from=0, to = 1, length.out=5), times = 4)
reapVals = rep(c(-1,1), each=10)
stratVals = rep(rep(c(-1,1), each=5), times=2);



pgamStratPOCreap = 1/(1+exp(-1*( (pocBeta*pocVals) + (stratReapBeta*stratVals*reapVals) + (pocStratReapBeta*pocVals*stratVals*reapVals))));

# 1:5: low reappraisal, natural
# 6:10: low reappraisal, strategy
# 11:15: high reappraisal, natural 
# 16:20: high reappraisal, strategy
...
plot(pgamStratPOCreap[11:15], type="l", lwd=6, col="goldenrod1", ylab="P(gamble)", xlab="Outcome(t-1) amount", cex=2, ylim=c(.29, .61), axes=F, main="high reap: poc x strat"); # high reappraisal natural
lines(pgamStratPOCreap[16:20], lwd=6, col="tomato1", lty=2); # avg reappraisal, strategy
axis(1, at=c(1,3,5), label=c("$0", "$30.50", "$61"), lwd=6, cex=1.5)
axis(2, at = c(.3, .4, .5, .6), labels= c(.30, .40, .50, .60), lwd =6, cex=1.5)
abline(a=.5, b=0, col="darkgrey", lty="dotted", lwd=6)
...
# look at indiv. level estimtes of poc x strategy and then plot against reap values
...
# add poc as randome effect in additinoa to poc:strat ranef
