...
# For RT – do group mean of subject-level median; could also see what happens when removing trials with very fast RTs
...
# No significant difference between natural and strategy for people who do natural and strategy first.
...
#RIGHT NOW THIS IS MEAN OF MEANS - DO MEAN OF MEDIANS
...
# Reaction times are very similar across instruction types.

# ACROSS ROUNDS
summary(rcsSubLevelWide_clean$round1_RT);
 #   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 # 0.8911  1.2170  1.4111  1.4171  1.6038  2.3205 

summary(rcsSubLevelWide_clean$round2_RT);
 #   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 # 0.9488  1.1198  1.3155  1.3118  1.4146  2.2312 

# Reaction times faster in round 2
...
cond1rtdiff = mean(natNatRound1rt- natNatRound2rt);
...
t.test(natNatRound1rt,natNatRound2rt, paired=T); # significant difference, p = .0001

# Participants are faster in round 2 when doing natural both rounds.
...
cond2rtdiff = mean(natStratRound1rt- natStratRound2rt);
...
t.test(natStratRound1rt,natStratRound2rt, paired=T); # significant difference, p = .009

# Participants are faster in round 2 when doing natural then strategy.
...
cond3rtdiff = mean(stratNatRound1rt- stratNatRound2rt);
...
t.test(stratNatRound1rt,stratNatRound2rt, paired=T); # significant difference, p =.0005

# participants significant faster in round 2 when doing strategy then natural
...
cond4rtdiff = mean(stratStratRound1rt- stratStratRound2rt);
...
t.test(stratStratRound1rt,stratStratRound2rt, paired=T); # not significant, p =.174

# RTs are not significant different when repeating strategy rounds, but direction of difference is consistent with above patterns.

# Overall, people are faster in round 2, regardless of condition/instructions.


# What about comparing strat vs nat in round 1 only:
natFirstRT = rcsSubLevelWide_clean$round1_RT[rcsSubLevelWide_clean$condCode %in% c(1,2)]; # people who do nat first

summary(natFirstRT);
 #   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 # 0.9459  1.2087  1.3528  1.4107  1.5822  2.3205 

stratFirstRT = rcsSubLevelWide_clean$round1_RT[rcsSubLevelWide_clean$condCode %in% c(3,4)]; # people who do strat first

summary(stratFirstRT);
 #   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 # 0.8911  1.2418  1.4595  1.4234  1.6064  2.0312 

t.test(natFirstRT, stratFirstRT); # ns, p= .79

# No significant difference between natural and strategy RTs for people who do natural and strategy first.



# Plot round 1 vs round 2 RT for each condition
pdf(file.path(config$path$directory, config$path$shlab_figures,'rtAcrossRndsStratPlots.pdf'))
par(pty="s", mfrow= c(2,2))
plot(natNatRound1rt, natNatRound2rt, ylim=c(0,3), xlim=c(0,3), ylab="Act natural Round 2", xlab="Act natural Round 1", pch=16, cex = 1.5, col="blue", main=sprintf("Natural-Natural\n RT diff = %.2f",cond1rtdiff))
abline(a=0, b=1, col="grey")

plot(natStratRound1rt, natStratRound2rt, ylim=c(0,3), xlim=c(0,3), ylab="Strategy Round 2", xlab="Act natural Round 1", pch=16, cex = 1.5, col="green", main=sprintf("Natural-Strategy\n RT diff = %.2f",cond2rtdiff))
abline(a=0, b=1, col="grey")

plot(stratStratRound1rt, stratStratRound2rt, ylim=c(0,3), xlim=c(0,3), ylab="Act Natural Round 2", xlab="Strategy Round 1", pch=16, cex = 1.5, col="red", main=sprintf("Strategy-Natural\n RT diff = %.2f",cond3rtdiff))
abline(a=0, b=1, col="grey")

plot(stratStratRound1rt, stratStratRound2rt, ylim=c(0,3), xlim=c(0,3), ylab="Strategy Round 2", xlab="Strategy Round 1", pch=16, cex = 1.5, col="purple", main=sprintf("Strategy-Strategy\n RT diff = %.2f",cond4rtdiff))
abline(a=0, b=1, col="grey")

mtext("Average Reaction Time", side = 3, line = - 2, outer = TRUE)
dev.off()

# At this point, we haven't removed any trials with fast RTs. How many trials do we have where participants were super fast:

sum(rdmDFclean$RT<1); # 7600 trials where participants were faster than 1s
sum(rdmDFclean$RT<.8); # 1884 trials where participants were faster than .8s
sum(rdmDFclean$RT<.5); # 21 trials where participants were faster than .5s


# let's see what happens to RT summary stats when we remove fast RTs. - the difference here is that we are looking at trial-level means where above is mean of means.
toofastTrialsInd = which(rdmDFclean$RT<1);
rdmDFclean$RTclean = rdmDFclean$RT
rdmDFclean$RTclean[toofastTrialsInd] = NA; # put nan on fast trials


# ACROSS INSTRUCTION TYPE
summary(rdmDFclean$RTclean[rdmDFclean$strategy==0]);
  #  Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
  # 1.000   1.164   1.357   1.500   1.675   3.992    4088 

summary(rdmDFclean$RTclean[rdmDFclean$strategy==1]);
  #  Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
  # 1.000   1.176   1.392   1.534   1.734   4.008    3512 

# Reaction times are similar across instruction types but slightly slower in strategy

# ACROSS ROUNDS
summary(rdmDFclean$RTclean[rdmDFclean$roundRDM==1]);
  #  Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
  # 1.000   1.190   1.412   1.557   1.756   4.008    3291 

summary(rdmDFclean$RTclean[rdmDFclean$roundRDM==2]);
  #  Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
  # 1.000   1.151   1.334   1.474   1.650   4.006    4309 
...
instDiff_model1 = lmer(instDifficult ~ 1+ compositeSpanScore + ERQreapp +  ERQsupp + strategyRecode + (1|subID), data=rcsSubLevelLong_clean)
summary(instDiff_model1); # no main effects, trending effect of strategy with higher difficulty in strategy condition

instDiff_model2 = lmer(instDifficult ~ 1+ compositeSpanScore*strategyRecode + ERQreapp*strategyRecode +  ERQsupp*strategyRecode+  (1|subID), data=rcsSubLevelLong_clean)
summary(instDiff_model2);
# no interaction with strategy

instHowOften_model1 = lmer(instHowOften ~ 1+ compositeSpanScore + ERQreapp +  ERQsupp + strategyRecode + (1|subID), data=rcsSubLevelLong_clean)
summary(instHowOften_model1);
# no main effects

instHowOften_model2 = lmer(instHowOften ~ 1+ compositeSpanScore*strategyRecode + ERQreapp*strategyRecode +  ERQsupp*strategyRecode+  (1|subID), data=rcsSubLevelLong_clean)
...
# left off here!! make esure to add round!!!
