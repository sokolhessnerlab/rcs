...
model_pocSignedShiftEarnExp_pred2 = glm(choice ~ 0 + pastOC1sc + signedShiftsc + earnNormalizedOverall + trialSC, data=rdmDFclean, family="binomial", offset=predModel2)
summary(model_pocSignedShiftEarnExp_pred2); 
# AIC = 30226
# all significant effects in expected directions

# check with positive and negative shift
model_pocPosNegShiftEarnExp_pred2 = glm(choice ~ 0 + pastOC1sc + posShiftsc + negShiftsc + earnNormalizedOverall + trialSC, data=rdmDFclean, family="binomial", offset=predModel2)
...
model_pocSignedShiftEarnExpIntxn_pred2 = glm(choice ~ 0 + pastOC1sc + signedShiftsc + earnNormalizedOverall*trialSC, data=rdmDFclean, family="binomial", offset=predModel2)
...
model_pocSignedShiftEarnExp_pocIntxn_pred2 = glm(choice ~ 0 + pastOC1sc + signedShiftsc + earnNormalizedOverall*pastOC1sc + trialSC*pastOC1sc, data=rdmDFclean, family="binomial", offset=predModel2)
...
model_pocPosNegShiftEarnExp_pocIntxn_pred2 = glm(choice ~ 0 + pastOC1sc*earnNormalizedOverall + posShiftsc + negShiftsc + trialSC*pastOC1sc,  data=rdmDFclean, family="binomial", offset=predModel2)
...
model_pocSignedShiftEarnExp_pocIntxnEarnOnly_pred2 = glm(choice ~ 0 + pastOC1sc*earnNormalizedOverall + signedShiftsc + trialSC,  data=rdmDFclean, family="binomial", offset=predModel2)
...
model_pocSignedShiftEarnExp_poc3wayIntxn_pred2 = glm(choice ~ 0 + pastOC1sc + signedShiftsc + earnNormalizedOverall*trialSC*pastOC1sc, data=rdmDFclean, family="binomial", offset=predModel2)
...
par(pty="s")
plot(pgamSignedShift, type="l", lwd=3, axes=F, ylim=c(.45, .55), ylab="p(gamble)", xlab="Shift amount", main="Effect size of shift on risk-taking (n=124)")
...
legend("bottomleft", legend=c("earn>exp", "earn=exp" , "earn<exp"), lty = 1, lwd=4, bty="n", col=c("gray60","gray33",  "black"), cex=1)
...
# PAST OUTCOME INTXN WITH STRATEGY
model_pocStrat_pred2 = glm(choice ~ 0 + pastOC1sc*strategyRecode*roundRecode, data=rdmDFclean, family="binomial", offset=predModel2)
summary(model_pocStrat_pred2)
# no interaction with strategy or main effect of strategy. poc is trending and negative

# Add strategy and round:
model_pocSignedShift_strategyRound_pred2 = glm(choice ~ 0 + pastOC1sc*strategyRecode*roundRecode + signedShiftsc*strategyRecode*roundRecode, data=rdmDFclean, family="binomial", offset=predModel2)
summary(model_pocSignedShift_strategyRound_pred2)
# positive effect of signed shift (more risk-taking as shift increases)
# trending interaction between signed shift and strategy; and trending intxn between signed shift, round and strat
# AIC: 30264


model_pocPosNegShift_strategyRound_pred2 = glm(choice ~ 0 + pastOC1sc*strategyRecode*roundRecode + posShiftsc*strategyRecode*roundRecode  + negShiftsc*strategyRecode*roundRecode, data=rdmDFclean, family="binomial", offset=predModel2)
summary(model_pocPosNegShift_strategyRound_pred2)  
# trending effect of negative shift (p=.08) -> less risk-taking as negative shift increases (gets more negative)
# trending(ish) effect of positive shift (p=.1) -> more risk-taking as positive shift increases
# intxn between positive shift and strategy -> positive shift effect stronger in strategy condition
# itxn between negative shift, round and strategy -> 
# AIC = 30263

# Better AIC with pos/neg shift with strategy and round

negShiftVals = rep(c(0,-.33),times=4)
rndVals = rep(c(-1,-1,1,1), times = 2)
stratVals = rep(c(-1,1), each=4)

negShiftBeta = 0.498119
rndBeta = -0.093954
strtBeta = -0.032511
strtRndBeta = 0.002929
strtNegShiftBeta = -0.093954
RndNegShiftBeta = -0.142059
strtRndNegShiftBeta = 0.793804


pgamNegShift_RoundxStrategy = 1/(1+exp(-1*( (negShiftBeta*negShiftVals) + (rndBeta*rndVals) + (strtBeta*stratVals) + (strtRndBeta*stratVals*rndVals) + (strtNegShiftBeta*stratVals*negShiftVals) + (RndNegShiftBeta*rndVals*negShiftVals) + (strtRndNegShiftBeta*stratVals*rndVals*negShiftVals) )));

plot(pgamNegShift_RoundxStrategy[1:2], type="l", lwd=3, ylim=c(.3,.6), col="red")
lines(pgamNegShift_RoundxStrategy[3:4], lwd=3, ylim=c(.3,.6), col="red", lty="dotted")
lines(pgamNegShift_RoundxStrategy[5:6], lwd=3, ylim=c(.3,.6), col="blue")
lines(pgamNegShift_RoundxStrategy[7:8], lwd=3, ylim=c(.3,.6), col="blue", lty="dotted")

# effects of natural round 1 and strategy round 2 are similar (direction is the same) whereas the effects of natural round 2 and strategy round 1 are also the similar (direction is same). 
# this might just be up against noise because of the few number of negative shift trials
# 
...
model_earnsExpIntxn_pred2 = glm(choice ~ 0 + earnNormalizedOverall*trialSC*strategyRecode, data=rdmDFclean, family="binomial", offset=predModel2)
summary(model_earnsExpIntxn_pred2)
...
model_earningsOverrounds_pred2 = glm(choice ~0 + earningsAcrossRounds*trialSC, data=rdmDFclean, family="binomial", offset=predModel2)
summary(model_earningsOverrounds_pred2); 
# no main effect of earnings across rounds but trending interaction with expectation
# AIC = 30513

# and including trial over all
model_earningsandExpOverrounds_pred2 = glm(choice ~ 0 + earningsAcrossRounds*trialAcrossRounds, data=rdmDFclean, family="binomial", offset=predModel2)
summary(model_earningsandExpOverrounds_pred2);
...
model_earnsOcIntxn_pred2 = glm(choice ~ 0 + earnNormalizedOverall*trialSC*pastOC1sc, data=rdmDFclean, family="binomial", offset=predModel2);
summary(model_earnsOcIntxn_pred2)
# when adding past outcome, both main effects of earnings and expectation goes away and so does the interaction
# AIC: 30219

model_earnsOC_expOC_Intxn_pred2 = glm(choice ~ 0 + earnNormalizedOverall*pastOC1sc+ trialSC*pastOC1sc, data=rdmDFclean, family="binomial", offset=predModel2)
summary(model_earnsOC_expOC_Intxn_pred2)
# AIC: 30216
...
model_earnsOCintxn_earnsExpintxn_pred2 = glm(choice ~ 0 + earnNormalizedOverall*pastOC1sc+ trialSC*earnNormalizedOverall, data=rdmDFclean, family="binomial", offset=predModel2)
summary(model_earnsOCintxn_earnsExpintxn_pred2)
# AIC = 30224
...
model_earnsOC_expOC_Intxn_strategy_pred2 = glm(choice ~ 0 + earnNormalizedOverall*pastOC1sc*strategyRecode+ trialSC*pastOC1sc*strategyRecode, data=rdmDFclean, family="binomial", offset=predModel2)
summary(model_earnsOC_expOC_Intxn_strategy_pred2)
# AIC: 30219
# no interaction with strategy


# PUT ALL TIMESCALES IN MODEL
model_3timescales_pred2 = glm(choice ~ 0 + pastOC1sc*earnNormalizedOverall + posShiftsc + negShiftsc + trialSC*pastOC1sc,  data=rdmDFclean, family="binomial", offset=predModel2)
summary(model_3timescales_pred2)
# AIC = 30212
...
model_3timescalesStrategy_pred2 = glm(choice ~ 0 + pastOC1sc*earnNormalizedOverall*strategyRecode + posShiftsc*strategyRecode + negShiftsc*strategyRecode + trialSC*pastOC1sc*strategyRecode,  data=rdmDFclean, family="binomial", offset=predModel2)
summary(model_3timescalesStrategy_pred2)
# AIC = 30213
...
model_3timescalesStrategyRound_pred2 = glm(choice ~ 0 + pastOC1sc*earnNormalizedOverall*strategyRecode*roundRDM + posShiftsc*strategyRecode*roundRDM + negShiftsc*strategyRecode*roundRDM + trialSC*pastOC1sc*strategyRecode*roundRDM,  data=rdmDFclean, family="binomial", offset=predModel2)
summary(model_3timescalesStrategyRound_pred2)
# AIC = 30219
