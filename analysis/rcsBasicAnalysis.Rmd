...
summary(erqCompSpan); # no relationship between suppression or reappraisal and composite span score
...
# Difficulty instructions not predicted by complex span, ERQ, motivation or strategy(trending tho). 
...
instHowOften_model2 = lmer(instHowOften ~ 1+ compositeSpanScore*strategyRecode + ERQreapp*strategyRecode +  ERQsupp*strategyRecode+ as.numeric(motivation)*strategyRecode +  (1|subID), data=rcsSubLevelLong_clean);
summary(instHowOften_model2);
# main effect of strategy (positive - higher frequency ratings in strategy condition)
# interaction between motivation and strategy (negative interaction - )
(-.69*1) + (-1*20.70981) + (1*-1*-3.601) = -14.88781 # low motivation, natural -> effect is negative on frequency
(-.69*7) + (-1*20.70981) + (7*-1*-3.601) = -0.33281 # high motivation, natural -> effect is less negative on frequency 
(-.69*1) + (1*20.70981) + (1*1*-3.601) = 12.12781 # low motivation, strategy -> effect is positive on frequency
(-.69*7) + (1*20.70981) + (7*1*-3.601) = -9.32719 # high motivation, strategy -> effect is negative on frequency (what?)

#break up the model by strategy and include round

rcsSubLevelLong_clean$roundRDM = rep(c(-1,1),times=nSub);  # round just gets dropped in the model

#LEFT OFF HERE
instHowOften_model4_natural = lm(instHowOften ~ 1+ compositeSpanScore*roundRDM + ERQreapp*roundRDM +  ERQsupp*roundRDM+ as.numeric(motivation)*roundRDM, data=rcsSubLevelLong_clean[rcsSubLevelLong_clean$strategyRecode==-1,]);
summary(instHowOften_model4_natural);
# in natural condition - no main effects of any of the variables or interactions


instHowOften_model5_strategy = lm(instHowOften ~ 1+ compositeSpanScore*roundRDM + ERQreapp*roundRDM +  ERQsupp*roundRDM+ as.numeric(motivation)*roundRDM, data=rcsSubLevelLong_clean[rcsSubLevelLong_clean$strategyRecode==1,]);
summary(instHowOften_model5_strategy);
# in strategy condition - main effect of motivation, ineteraction between round and ERQ suppression and trending effect of ERQ reapp
# negative effect of motivation on frequency ratings - lower motivation, higher frequency ratings?


# Do inst difficulty next
