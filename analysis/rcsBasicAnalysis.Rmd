...
mean(c(lowreapdf$round1_instDifficult[lowreapdf$round1_strategy==1],lowreapdf$round2_instDifficult[lowreapdf$round2_strategy==1]), na.rm=T);

mean(c(modhighreapdf$round1_instDifficult[modhighreapdf$round1_strategy==1],modhighreapdf$round2_instDifficult[modhighreapdf$round2_strategy==1]), na.rm=T);

instDiff_reapthirds_strategy = lmer(instDifficultSC ~ 1+  lowReapTertile*strategyRecode + highReapTertile*strategyRecode+ (1|subID), data=rcsSubLevelLong_clean);
summary(instDiff_reapthirds_strategy);
# effect of strategy (high difficulty ratings with strategy) and trending interaction between strategy and high reap which is in the direction that : .04*1 -.06*1 = -.02 net effect on difficulty ratings for high reapraisal people in strategy condition suggesting that for high reappraisers they may have found the strategy easier.

instFreq_reapthirds_strategy = lmer(instHowOftenSC ~ 1+  lowReapTertile*strategyRecode + highReapTertile*strategyRecode+ (1|subID), data=rcsSubLevelLong_clean);
summary(instFreq_reapthirds_strategy);
# no effects

# overall, no strong indication that reappraisal was related to difficulty or frequency ratings.
