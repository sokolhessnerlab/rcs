...
# base model 1
pocShiftEarnExp_ERQreapXstratXpoc = glm(choice~0 + pastOC1sc*strategyRecode*ERQreappSC + signedShiftsc + earnNormalizedOverall + linExpectation, data=rdmDFclean, family="binomial",offset=predModel2);
summary(pocShiftEarnExp_ERQreapXstratXpoc);
# AIC: 29757 slightly worse AIC removing strat and ERQ from other timescales but the results are similar and now we are seeing the main effects of temporal context as expected.

# base model 2:
pocShiftEarnExp_pocIntnx_ERQreapXstratXpoc = glm(choice~0 + pastOC1sc*strategyRecode*ERQreappSC + linExpectation*pastOC1sc + signedShiftsc + pastOC1sc*earnNormalizedOverall, data=rdmDFclean, family="binomial",offset=predModel2);
summary(pocShiftEarnExp_pocIntnx_ERQreapXstratXpoc);
# AIC: 29744 - removing the interactions with ERQ at shift and global timescale improves model
