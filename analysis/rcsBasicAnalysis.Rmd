...
RT_roundStrat= lmer(sqrt(RT) ~ roundRDM + strategyRecode + roundRDM*strategyRecode + (1|subID), data= rdmDFclean);
# only significant effect of round - with faster RTs in round 2
summary(RT_roundStrat); # using all RTs (not median RTs) in a lmer model, there are significant effects of round and strategy and an interaction between round and strategy
# faster RTs in round 2 and in ignore context condition
# positive interaction: 
# round 1, nat: -.04*-1 + -.015*-1 +  .01*-1*-1 = .065 weight on RT
# round 1, strat: -.04*-1 + -.015*1 +  .01*-1*1 = .015 weight on RT
# round 2, nat: -.04*1 + -.015*-1 +  .01*-1*1 = -.035 weight on RT
# round 2, strat: -.04*1 + -.015*1 +  .01*1*1 = -.045 weight on RT

# everyone is faster in round 2 regardless of strategy but within rounds, people are faster in strategy


RT_roundStrat_500ms= lmer(sqrt(RT) ~ roundRDM + strategyRecode + roundRDM*strategyRecode + (1|subID), data= rdmDFclean[rdmDFclean$RT>.5,]);
summary(RT_roundStrat_500ms); # same effect/beta values when removing super fast trials
...
median(sqrt(rdmDFclean$RT[rdmDFclean$RT>.5 & rdmDFclean$strategyRecode==-1])); #1.220154
mean(sqrt(rdmDFclean$RT[rdmDFclean$RT>.5 & rdmDFclean$strategyRecode==-1])); #1.220154

median(sqrt(rdmDFclean$RT[rdmDFclean$RT>.5 & rdmDFclean$strategyRecode==1]));  # 1.262684

mean(sqrt(rdmDFclean$RT[rdmDFclean$RT>.5 & rdmDFclean$strategyRecode==1]));  #


median(rdmDFclean$RT[rdmDFclean$RT>.5 & rdmDFclean$roundRecode==-1], na.rm=T); #1.294482
median(rdmDFclean$RT[rdmDFclean$RT>.5 & rdmDFclean$roundRecode==1]);  # 1.197061
...
sd(rcsSubLevelWide_clean$motivationNumeric[rcsSubLevelWide_clean$condCode==1]); # sd = 0.9481164
summary(rcsSubLevelWide_clean$motivationNumeric[rcsSubLevelWide_clean$condCode==2]); # mean = 5.26
sd(rcsSubLevelWide_clean$motivationNumeric[rcsSubLevelWide_clean$condCode==2]); # sd = 0.9989242
summary(rcsSubLevelWide_clean$motivationNumeric[rcsSubLevelWide_clean$condCode==3]); # mean = 5.34
sd(rcsSubLevelWide_clean$motivationNumeric[rcsSubLevelWide_clean$condCode==3]); # sd = 1.141589
summary(rcsSubLevelWide_clean$motivationNumeric[rcsSubLevelWide_clean$condCode==4]); # mean = 5.1
sd(rcsSubLevelWide_clean$motivationNumeric[rcsSubLevelWide_clean$condCode==4]); # mean = 1.359949
...
sd(rcsSubLevelWide_clean$rdmRoundsIndepNumeric[rcsSubLevelWide_clean$condCode==1]); # sd = 1.877427
summary(rcsSubLevelWide_clean$rdmRoundsIndepNumeric[rcsSubLevelWide_clean$condCode==2]); # mean = 5.355
sd(rcsSubLevelWide_clean$rdmRoundsIndepNumeric[rcsSubLevelWide_clean$condCode==2]); # sd= 1.226061
summary(rcsSubLevelWide_clean$rdmRoundsIndepNumeric[rcsSubLevelWide_clean$condCode==3]); # mean = 4.871
sd(rcsSubLevelWide_clean$rdmRoundsIndepNumeric[rcsSubLevelWide_clean$condCode==3]); # sd=1.707668
summary(rcsSubLevelWide_clean$rdmRoundsIndepNumeric[rcsSubLevelWide_clean$condCode==4]); # mean = 5.258
sd(rcsSubLevelWide_clean$rdmRoundsIndepNumeric[rcsSubLevelWide_clean$condCode==4]); # sd = 1.210172

roundIndepRoundStrat = lmer(as.numeric(rcsSubLevelLong_clean$rdmRoundsIndep)~strategyRecode*rdmRound + (1|subID), data=rcsSubLevelLong_clean);
...
```{r complex-span-tasks-ERQ}
 ...
 sd(rcsSubLevelWide_clean$compositeSpanScore, na.rm=T); # range = 0.06 - 1; median = .5743, mean = .5720, sd = 0.2236828
...
# ERQ
 summary(rcsSubLevelWide_clean$ERQreappSum); # range = 14-42; mean = 29.69; median =30
 sd(rcsSubLevelWide_clean$ERQreappSum, na.rm=T); # sd = 5.487901
 summary(rcsSubLevelWide_clean$ERQsuppSum); # range = 4 - 27; mean = 14.89; median = 15
 sd(rcsSubLevelWide_clean$ERQsuppSum, na.rm=T) # sd = 5.497102

 summary(rcsSubLevelWide_clean$ERQreappMean); # range = 2.333- 7; mean = 4.948, median = 5.000
 sd(rcsSubLevelWide_clean$ERQreappMean, na.rm=T); # sd = 0.9146502
 summary(rcsSubLevelWide_clean$ERQsuppMean); # range = 1 - 6.75; mean = 3.721; median = 3.75
 sd(rcsSubLevelWide_clean$ERQsuppMean, na.rm=T); # sd = 1.374275


#is complex span realted to ERQ 
cor.test(rcsSubLevelWide_clean$compositeSpanScore, rcsSubLevelWide_clean$ERQreappMean); # r = .032(97), p = .75
cor.test(rcsSubLevelWide_clean$compositeSpanScore, rcsSubLevelWide_clean$ERQsuppMean); # r = .045(97), p = .66

 
pdf(file.path(config$path$directory, config$path$shlab_figures,'ERQHist_sums.pdf'))
par(mfrow=c(2,1))
hist(rcsSubLevelWide_clean$ERQreappSum, breaks=seq(from=13.5, to = 42.5, by = 1), xlab="ERQ Reappraisal Sums", main="ERQ Reappraisal (n=122)\nmean = 29.7, range = 14-42", ylim=c(0,15), axes=F)
axis(1, at = seq(14,42, by=2))
axis(2)
hist(rcsSubLevelWide_clean$ERQsuppSum, breaks=seq(from=3.5, to = 27.5, by = 1),  xlab="ERQ Suppression Sums", main="ERQ Suppression (n=122)\nmean = 14.9, range = 4-27", ylim=c(0,15), axes=F)
axis(1, at = seq(4, 28, by=2))
axis(2)
dev.off()


pdf(file.path(config$path$directory, config$path$shlab_figures,'ERQHist_means.pdf'))
par(mfrow=c(2,1))
hist(rcsSubLevelWide_clean$ERQreappMean, breaks=seq(from=1.5, to = 7.5, length.out=25), xlab="ERQ Reappraisal Scores", main="ERQ Reappraisal (n=122)\nmean = 4.95, range = 2.33-7", axes=F)
axis(1, at = c(2.33,7))
axis(2)
hist(rcsSubLevelWide_clean$ERQsuppMean, breaks=seq(from=.5, to = 7.25, length.out =25),  xlab="ERQ Suppression Scores", main="ERQ Suppression (n=122)\nmean = 3.72, range = 1-6.75", axes=F)
axis(1, at = c(1, 6.75))
axis(2)
dev.off()
...
cor.test(rcsSubLevelWide_clean$ERQreappMean, rcsSubLevelWide_clean$ERQsuppMean, paired=T); #r = -.07(120), p =.4

cor.test(rcsSubLevelWide_clean$ERQreappMean, rcsSubLevelWide_clean$motivationNumeric, paired=T); #r = -.03(120), p =.75
cor.test(rcsSubLevelWide_clean$ERQsuppMean, rcsSubLevelWide_clean$motivationNumeric, paired=T); #r = -.22(120), p =.01

# Is ERQ related to composite span?
erqCompSpan = lm(rcsSubLevelWide_clean$compositeSpanScore~rcsSubLevelWide_clean$ERQreappMean + rcsSubLevelWide_clean$ERQsuppMean, data=rcsSubLevelWide_clean);
...
erqMotivation = lm(rcsSubLevelWide_clean$motivationNumeric~rcsSubLevelWide_clean$ERQreappMean + rcsSubLevelWide_clean$ERQsuppMean, data=rcsSubLevelWide_clean);
...
rcsSubLevelLong_clean$ERQreappSC = rcsSubLevelLong_clean$ERQreappMean/max(rcsSubLevelLong_clean$ERQreappMean, na.rm=T)
rcsSubLevelLong_clean$ERQsuppSC = rcsSubLevelLong_clean$ERQsuppMean/max(rcsSubLevelLong_clean$ERQsuppMean, na.rm=T)
...
instDiff_SpanERQMotiv_strategyRound = lmer(instDifficultSC ~ 1+ compositeSpanScore*condCode + ERQreappSC*condCode +  ERQsuppSC*condCode+ motivationSC*condCode + (1|subID), data= rcsSubLevelLong_clean);
summary(instDiff_SpanERQMotiv_strategyRound); # no main effect of condition code

instFreq_SpanERQMotiv_strategyRound = lmer(instHowOftenSC ~ 1+ compositeSpanScore*condCode + ERQreappSC*condCode +  ERQsuppSC*condCode+ motivationSC*condCode + (1|subID), data= rcsSubLevelLong_clean);
...
instDiff_SpanERQMotiv_strategy_round1data = lm(instDifficultSC ~ 1+ compositeSpanScore*strategyRecode + ERQreappSC*strategyRecode +  ERQsuppSC*strategyRecode+ motivationSC*strategyRecode, data= rcsSubLevelLong_clean[rcsSubLevelLong_clean$roundRDM==-1,]);
...
instDiff_SpanERQMotiv_strategy_round2data = lm(instDifficultSC ~ 1+ compositeSpanScore*strategyRecode + ERQreappSC*strategyRecode +  ERQsuppSC*strategyRecode+ motivationSC*strategyRecode, data= rcsSubLevelLong_clean[rcsSubLevelLong_clean$roundRDM==1,]);
...
instFreq_SpanERQMotiv_strategy_round1data = lm(instHowOftenSC ~ 1+ compositeSpanScore*strategyRecode + ERQreappSC*strategyRecode +  ERQsuppSC*strategyRecode+ motivationSC*strategyRecode, data= rcsSubLevelLong_clean[rcsSubLevelLong_clean$roundRDM==-1,]);
...
instFreq_SpanERQMotiv_strategy_round2data = lm(instHowOftenSC ~ 1+ compositeSpanScore*strategyRecode + ERQreappSC*strategyRecode +  ERQsuppSC*strategyRecode+ motivationSC*strategyRecode, data= rcsSubLevelLong_clean[rcsSubLevelLong_clean$roundRDM==1,]);
...
# using predicted values from model 3 above (gain, safe, ev, round and strategy)
...
# AIC = 30226 (248 observations deleted due to missingness)
# all significant effects in expected directions - best AIC of these 3 models


# check with earnings and expectations across rounds
model_pocSignedShiftEarnExpAcrossRounds_pred2 = glm(choice ~ 0 + pastOC1sc + signedShiftsc + earningsAcrossRounds + linExpAcrossRounds, data=rdmDFclean, family="binomial", offset=predModel2)
summary(model_pocSignedShiftEarnExpAcrossRounds_pred2); 
# AIC = 30237 - AIC is worse than model with earnings and expectations across rounds, but the effects are significant

model_pocSignedEarnExpWithinAndAcrossRounds_pred2 = glm(choice ~ 0 + pastOC1sc + signedShiftsc + earnNormalizedOverall+ linExpectation+ earningsAcrossRounds + linExpAcrossRounds, data=rdmDFclean, family="binomial", offset=predModel2)
summary(model_pocSignedEarnExpWithinAndAcrossRounds_pred2); 
# AIC = 30229
# only effects of earnings and expectation (Trending) are present
...
#just positive shift
model_pocPosShiftEarnExp_pred2 = glm(choice ~ 0 + pastOC1sc + posShiftsc + earnNormalizedOverall + linExpectation, data=rdmDFclean, family="binomial", offset=predModel2)
summary(model_pocPosShiftEarnExp_pred2); 
#aic = 30230
...
# AIC = 30211 (248 observations deleted due to missingness)
...
par(pty="s")
plot(pgamPOC, type="l", lwd=8, axes=F, ylim=c(.4, .6), ylab="p(gamble)", xlab="Outcome (t-1) amount", main="Effect size of past outcome on risk-taking (n=124)")
axis(1, at = c(1,3,5), labels = c("$0", "$30.50", "$60.98"), lwd=8)
axis(2, at = c(.6,.55, .5, .45, .4), labels = c(.60,.55, .50,.45, .40), lwd=8)
abline(h=0.5, col="grey", lwd=8, lty="dotted")
dev.off()
...
par(pty="s")
plot(pgamSignedShift, type="l", lwd=8, axes=F, ylim=c(.4, .6), ylab="p(gamble)", xlab="Shift amount", main="Effect size of signed shift on risk-taking (n=124)")
axis(1, at = c(1:5), labels = c("-$20", "-$10", "$0", "+$10", "+$20"), lwd=8)
axis(2, c(.6,.55, .5, .45, .4), labels = c(.60,.55, .50,.45, .40), lwd=8)
abline(h=0.5, col="grey", lwd=8, lty="dotted")
dev.off()
...
plot(PgamSub1EarnExp, type="l", col="darkgreen", lwd=8, ylim=c(.4,.6), ylab="p(gamble)", xlab="trial", main="risk taking as a function\n of earnings and expectations; round1", axes=F, cex.lab=1, xlim=c(1,131))
abline(a=.5, b=0, lty="dotted", col="darkgrey", lwd=8)
#lines(PgamSub2EarnExp, col="darkred", lwd=6);
#lines(PgamSub3EarnExp, col="darkblue", lwd=6);
lines(PgamSub4EarnExp, col="darkred", lwd=8);
lines(PgamSub5EarnExp, col="darkblue", lwd=8);
lines(PgamSub6EarnExp, col="darkorange", lwd=8);
#lines(PgamSub7EarnExp, col="purple4", lwd=6);


legend("bottomright", legend=c("sub 1", "sub 71","sub 86", "sub 116"), lty = 1, lwd=8, bty="n", col=c("darkgreen","darkred",  "darkblue", "darkorange"), cex=.75)

#legend("bottomright", legend=c("sub 1", "sub 24" , "sub 40","sub 71","sub 86", "sub 116", "sub 132"), lty = 1, lwd=6, bty="n", col=c("darkgreen","darkred",  "darkblue", "darkorange","goldenrod","cadetblue", "purple4"), cex=.75)
axis(1, at=c(1,131),tick =T, cex.axis = 1.25, cex=2, lwd = 8)
axis(2, tick =T, cex.axis = 1.25, cex=2, lwd = 8)
dev.off()
...
# Does motivation, ERQ and complex span interact with strategy?
# base model 1
pocShiftEarnExp_ERQspanMotivxStrat = glm(choice~0 + pastOC1sc*strategyRecode + signedShiftsc*strategyRecode + earnNormalizedOverall*strategyRecode + linExpectation*strategyRecode + suppSpan0mean*strategyRecode + reapSpan0mean*strategyRecode + motivationNumeric*strategyRecode + compositeSpanScore*strategyRecode, data=rdmDFclean, family="binomial",offset=predModel2);
summary(pocShiftEarnExp_ERQspanMotivxStrat);
# AIC: 23818 - much lower because loss of data
# only interaction between strategy and reappraisal is trending

# # base model 2
pocShiftEarnExp_pocIntnx_ERQspanMotivxStrat = glm(choice~0 + pastOC1sc*linExpectation*strategyRecode + signedShiftsc*strategyRecode + pastOC1sc*earnNormalizedOverall*strategyRecode + suppSpan0mean*strategyRecode + reapSpan0mean*strategyRecode + motivationNumeric*strategyRecode + compositeSpanScore*strategyRecode, data=rdmDFclean, family="binomial",offset=predModel2);
summary(pocShiftEarnExp_pocIntnx_ERQspanMotivxStrat);
# AIC: 23810
# main effects hold up and only reappraisal interacts with strategy


# SUMMARY: of our individual-level measures, only ERQ reappraisal interacts with strategy and the models are better performing with just reappraisal. Results are similar across base model 1 and 2


# Check trial-level to see if ERQ, motivation and span interact with strategy (before accounting for temporal context effects)
...
# AIC = 30641.3 
# strongest effect is of reappraisal on strategy (interaction is negative, reap beta is positive)
# trending interaction of supp on strategy (interaction is negative, supp beta is negative)
...
# # AIC = 30641.7, removing suppression increased AIC slightly.
# 
...
# LOOK AT ERQ IN CONTEXTUAL MODELS
# unclear if we should be accounting for ERQ at the trial-level because we are interested in what happens with strategy and temporal context
# add reappraisal and suppression to our contextual base models 1 and 2

# just interaction ERQ supp and reap with strategy
# base model 1
pocShiftEarnExp_ERQxStratyOnly = glm(choice~0 + pastOC1sc*strategyRecode + signedShiftsc*strategyRecode + earnNormalizedOverall*strategyRecode + linExpectation*strategyRecode + suppSpan0mean*strategyRecode + reapSpan0mean*strategyRecode, data=rdmDFclean, family="binomial",offset=predModel2);
summary(pocShiftEarnExp_ERQxStratyOnly);
# AIC = 29765
# significant interaction between strategy and ERQreappraisal. 
# no interaction with suppression
# main contextual effects still present


# base model 2
pocShiftEarnExp_pocIntnx_ERQxStratOnly = glm(choice~0 + pastOC1sc*linExpectation*strategyRecode + signedShiftsc*strategyRecode + pastOC1sc*earnNormalizedOverall*strategyRecode + suppSpan0mean*strategyRecode + reapSpan0mean*strategyRecode, data=rdmDFclean, family="binomial",offset=predModel2);
summary(pocShiftEarnExp_pocIntnx_ERQxStratOnly);
# AIC: 29753
# main effects still there, only reap interacts with strategy

# between base model 1 and 2, only reappraisal (and not suppression) interacts with strategy
# interaction is negative in both models (and reap beta is positive)

# just include reappraisal
# base model 1
pocShiftEarnExp_ERQreapXstratOnly = glm(choice~0 + pastOC1sc*strategyRecode + signedShiftsc*strategyRecode + earnNormalizedOverall*strategyRecode + linExpectation*strategyRecode + reapSpan0mean*strategyRecode, data=rdmDFclean, family="binomial",offset=predModel2);
summary(pocShiftEarnExp_ERQreapXstratOnly);
# AIC = 29763 - removing supp from base model 1 improves AIC
# significant interaction between strategy and ERQreappraisal. 
# no interaction with suppression
# main contextual effects still present
...
# base model 2
pocShiftEarnExp_pocIntnx_ERQreapXstratOnly = glm(choice~0 + pastOC1sc*linExpectation*strategyRecode + signedShiftsc*strategyRecode + pastOC1sc*earnNormalizedOverall*strategyRecode + reapSpan0mean*strategyRecode, data=rdmDFclean, family="binomial",offset=predModel2);
summary(pocShiftEarnExp_pocIntnx_ERQreapXstratOnly);
#AIC = 29751- removing supp has better AIC for model 2 as well


# Now let's look at whether ERQ interacts with strategy at each level of context in both base models
# base model 1
pocShiftEarnExp_ERQreapXstrat_3timescales = glm(choice~0 + pastOC1sc*strategyRecode*reapSpan0mean + signedShiftsc*strategyRecode*reapSpan0mean + earnNormalizedOverall*strategyRecode*reapSpan0mean + linExpectation*strategyRecode*reapSpan0mean, data=rdmDFclean, family="binomial",offset=predModel2);
summary(pocShiftEarnExp_ERQreapXstrat_3timescales);
# AIC = 29756- interacting reap w. strat improves model
# significant interaction between poc x strat x reap
# still main effect of erq x strat
# now interaction bewteen poc and reap
# interaction bewteen strat and poc
# most other main effects go away except strat and poc (no global timescale interactions)

# base model 2
pocShiftEarnExp_pocIntnx_ERQreapXstrat_3timescales = glm(choice~0 + pastOC1sc*linExpectation*strategyRecode*reapSpan0mean + signedShiftsc*strategyRecode*reapSpan0mean + pastOC1sc*earnNormalizedOverall*strategyRecode*reapSpan0mean, data=rdmDFclean, family="binomial",offset=predModel2);
summary(pocShiftEarnExp_pocIntnx_ERQreapXstrat_3timescales);
#AIC = 29748-  adding erq reap interaction to strartegy improves model
# similar to base model 1 right above: poc x reap, strat x erq, and 3-way bt poc x strat x reap
# all other main effects go away

# because there isn't an interaction between strategy x erq and other timescales besides poc, let's remove those interactions from the models:

# base model 1
pocShiftEarnExp_ERQreapXstratXpoc = glm(choice~0 + pastOC1sc*strategyRecode*reapSpan0mean + signedShiftsc*strategyRecode + earnNormalizedOverall*strategyRecode + linExpectation*strategyRecode, data=rdmDFclean, family="binomial",offset=predModel2);
summary(pocShiftEarnExp_ERQreapXstratXpoc);
# AIC: 29754 better AIC removing strat and ERQ from global timescales but the results are similar and now we are seeing the main effects of temporal context as expected.
# still seeing interaction between, strat x reap, strat x poc x reap

# high reap, nat: -.2 + 1*.3 + 1*-1*.49 = -.39
# high reap, strat: -.2 + 1*.3 + 1*1*.49 = .59
# low reap, nat: -.2 + -1*.3 + -1*-1*.49 = -.01
# low reap, strat: -.2 + -1*.3 + -1*1*.49 = -.99
# avg reap, nat: -.2 + 0*.3 + 0*-1*.49 = -.2
# avg reap, strat: -.2 + 0*.3 + 0*1*.49 = -.2

# AIC: 29754
# people who are low on reappraisal -> there is no past outcome effect in act natural, but effect of past outcome is exacerbated in strategy.
# average person -> negative past outcome effect, there's no intnx bt strategy and reappraisal
# for people who are high on reappraisal -> pattern of past outcome effect reverse (overcompensating?)
# reappraisal only interacts with immediate timescale
# strategic control has limited ability to influence context effects
# at other timescales, the effects are not happening as a result of intentional reflection and may be so fundamental that they are not aware these are happening.
# people's habitual experience of controlling their thoughts is a very important factor to this picture.
# these are the big picture things to focus on - will need to unpack those little pieces but come back to these important big picture things
# will want to nod that we want to be careful about null effects and that we would want to follow up on those later but that we can see some changes suggests that we are powered to see others (and should take seriosulys the null effects)

# what does this tell us about context effects?
# what does this tell us about capacity?
# what does this tell us about the connection to affect?



# base model 2:
pocShiftEarnExp_pocIntnx_ERQreapXstratXpoc = glm(choice~0 + pastOC1sc*strategyRecode*reapSpan0mean + linExpectation*pastOC1sc + signedShiftsc*strategyRecode + pastOC1sc*earnNormalizedOverall, data=rdmDFclean, family="binomial",offset=predModel2);
summary(pocShiftEarnExp_pocIntnx_ERQreapXstratXpoc);
# AAIC: 29741 - removing the interactions with ERQ at global timescale improves model
# results the same as above with main effects at each timescale and all interactions betwee poc, strat and erq reap



# PLOT THESE RESULTS:
# basemodel1_erqStrat_temporalcontext  = summary(pocShiftEarnExp_ERQreapXstratXpoc)
# pocBeta = basemodel1_erqStrat_temporalcontext$coefficients[1]
# stratBeta =  basemodel1_erqStrat_temporalcontext$coefficients[2]
# reapBeta =  basemodel1_erqStrat_temporalcontext$coefficients[3]
# pocStratBeta = basemodel1_erqStrat_temporalcontext$coefficients[7]
# pocReapBeta = basemodel1_erqStrat_temporalcontext$coefficients[8]
# stratReapBeta = basemodel1_erqStrat_temporalcontext$coefficients[9]
# pocStratReapBeta = basemodel1_erqStrat_temporalcontext$coefficients[13]

basemodel1_erqStrat_temporalcontext  = summary(pocShiftEarnExp_ERQreapXstrat_3timescales); # using model with ERQ x 3 timescales
pocBeta = basemodel1_erqStrat_temporalcontext$coefficients[1]
stratBeta =  basemodel1_erqStrat_temporalcontext$coefficients[2]
reapBeta =  basemodel1_erqStrat_temporalcontext$coefficients[3]
pocStratBeta = basemodel1_erqStrat_temporalcontext$coefficients[7]
pocReapBeta = basemodel1_erqStrat_temporalcontext$coefficients[8]
stratReapBeta = basemodel1_erqStrat_temporalcontext$coefficients[9]
pocStratReapBeta = basemodel1_erqStrat_temporalcontext$coefficients[16]


# plot results of poc x strategy x reappraisal at different levels of reappraisal -> low, middle, high
# each figure will have two lines for strategy and reappraisal
pocVals = rep(seq(from=0, to = 1, length.out=5), times = 6)
reapVals = rep(c(-1,0,1), each=10)
stratVals = rep(rep(c(-1,1), each=5), times=3);



pgamStratPOCreap = 1/(1+exp(-1*( (pocBeta*pocVals) + (pocReapBeta*pocVals*reapVals) + (stratReapBeta*stratVals*reapVals) + (pocStratReapBeta*pocVals*stratVals*reapVals))));

# 1:5: low reappraisal, natural
# 6:10: low reappraisal, strategy
# 11:15: avg reappraisal, natural 
# 16:20: avg reappraisal, strategy
# 21:25 high reappraisal, natural
# 26:30 high reappraisal, strategy


# low reappraisal
pdf(file.path(config$path$directory, config$path$shlab_figures,'model_basemodel1_lowreap_pocStratES.pdf'))
plot(pgamStratPOCreap[1:5], type="l", lwd=6, col="goldenrod1", ylab="P(gamble)", xlab="Outcome(t-1) amount", cex=2, ylim=c(.29, .61), axes=F, main="low reap: poc x strat"); # low reappraisal natural
lines(pgamStratPOCreap[6:10], lwd=6, col="tomato1", lty=2); # low reappraisal, strategy

axis(1, at=c(1,3,5), label=c("$0", "$30.50", "$61"), lwd=6, cex=1.5)
axis(2, at = c(.3, .4, .5, .6), labels= c(.30, .40, .50, .60), lwd =6, cex=1.5)
abline(a=.5, b=0, col="darkgrey", lty="dotted", lwd=6)
legend("bottomleft", legend=c("Act Natural","Ignore Context"),lty=c(1,2), bty="n", col=c("goldenrod1", "tomato1"), cex=1.5, lwd=6)
dev.off()


# avg reappraisal
pdf(file.path(config$path$directory, config$path$shlab_figures,'model_basemodel1_avgreap_pocStratES.pdf'))
plot(pgamStratPOCreap[11:15], type="l", lwd=6, col="goldenrod1", ylab="P(gamble)", xlab="Outcome(t-1) amount", cex=2, ylim=c(.29, .61), axes=F, main="avg reap: poc x strat"); # high reappraisal natural
lines(pgamStratPOCreap[16:20], lwd=6, col="tomato1", lty=2); # avg reappraisal, strategy
axis(1, at=c(1,3,5), label=c("$0", "$30.50", "$61"), lwd=6, cex=1.5)
axis(2, at = c(.3, .4, .5, .6), labels= c(.30, .40, .50, .60), lwd =6, cex=1.5)
abline(a=.5, b=0, col="darkgrey", lty="dotted", lwd=6)
legend("bottomleft", legend=c("Act Natural","Ignore Context"),lty=c(1,2), bty="n", col=c("goldenrod1", "tomato1"), cex=1.5, lwd=6)
...
# high reappraisal
pdf(file.path(config$path$directory, config$path$shlab_figures,'model_basemodel1_highreap_pocStratES.pdf'))
plot(pgamStratPOCreap[21:25], type="l", lwd=6, col="goldenrod1", ylab="P(gamble)", xlab="Outcome(t-1) amount", cex=2, ylim=c(.29, .61), axes=F, main="high reap: poc x strat"); # high reappraisal natural
lines(pgamStratPOCreap[26:30], lwd=6, col="tomato1", lty=2); # high reappraisal, strategy
axis(1, at=c(1,3,5), label=c("$0", "$30.50", "$61"), lwd=6, cex=1.5)
axis(2, at = c(.3, .4, .5, .6), labels= c(.30, .40, .50, .60), lwd =6, cex=1.5)
abline(a=.5, b=0, col="darkgrey", lty="dotted", lwd=6)
legend("bottomleft", legend=c("Act Natural","Ignore Context"),lty=c(1,2), bty="n", col=c("goldenrod1", "tomato1"), cex=1.5, lwd=6)
...
```{r strat-tempContext-ERQcategorical}
# That low reappraisers have a weird pattern (no past outcome effect in natural and super negative in strategy) suggests that the model is missing some nonlinear things that could be happening
# Let's split groups into 1/3 low, middle and high reappraisers and then put two variables in the model with high and low (and moderate people are the main estimates that we are comparing to)
# we can also check median split with two variables (high and low where yes = 1 and no = 0). But we only put one of these (.e.g high reap) in the model and then the main estimates are the low group that we compare with high

summary(rcsSubLevelWide_clean$ERQreappMean)


pocShiftEarnExp_ERQreapXstratXpoc_highLowReapMedSplit = glm(choice~0 + pastOC1sc*strategyRecode*isHighReapMedSplit + signedShiftsc*strategyRecode + earnNormalizedOverall*strategyRecode + linExpectation*strategyRecode, data=rdmDFclean, family="binomial",offset=predModel2);
summary(pocShiftEarnExp_ERQreapXstratXpoc_highLowReapMedSplit);
# AIC: 29757
# main estimates are for LOW reappraisers
# poc effect for low reappraisers = -.25
# poc effect for high reappraisers = -.25 + 0.16 = -.09

# poc x strategy effect of low reappraisers = -.1
# poc x strategy effect for high reappraisers = -.1 + 0.345309 = .25


# Try top and bottom third reap (comparing everything to middle)
pocShiftEarnExp_ERQreapXstratXpoc_highLowReap3rdsplit = glm(choice~0 + pastOC1sc*strategyRecode*highReapTopThird +pastOC1sc*strategyRecode*lowReapBottomThird + signedShiftsc*strategyRecode + earnNormalizedOverall*strategyRecode + linExpectation*strategyRecode, data=rdmDFclean, family="binomial",offset=predModel2);
summary(pocShiftEarnExp_ERQreapXstratXpoc_highLowReap3rdsplit);
# AIC = 29748 - best AIC so far
# only interactions with low reappraisal are significant

# MODERATE: poc + poc x strategy (+1/-1)
# -0.188832 + 0.376590*-1 # -0.565422 natural - using ERQ sum in model
-0.192829 + 0.290295*-1 # -0.483124 natural
#-0.188832 + 0.376590*1 #  0.187758 strategy  - using ERQ sum in model
-0.192829 + 0.290295*1 # 0.097466 strategy

# HIGH: poc + poc x strategy (+1/-1) + poc x high + poc x high x strategy (+1/-1)
# -0.188832 + 0.376590*-1 + 0.175040 + -0.193435*-1 # -0.196947 natural using ERQ sum in model
-0.192829 + 0.290295*-1 + 0.116045 + -0.031371*-1 # -0.335708 natural 
#-0.188832 + 0.376590*1 + 0.175040 + -0.193435*1 # 0.169363 strategy using ERQ sum in model
-0.192829 + 0.290295*1 + 0.116045 + -0.031371*1 # 0.18214 strategy
# so there is a dispositional thing here where starting point is different

# LOW: poc + poc x strategy (+1/-1) + poc x low + poc x low x strategy (+1/-1)
#-0.188832 + 0.376590*-1 + -0.196764 + -0.622534*-1 # natural -0.139652 using ERQ sum in model
-0.192829 + 0.290295*-1 + 0.007829 + -0.345870*-1 # natural -0.129425
#-0.188832 + 0.376590*1 + -0.196764 + -0.622534* 1 # strategy -0.63154 using ERQ sum in model
-0.192829 + 0.290295*1 + 0.007829 + -0.345870*1 # strategy -0.240575 

# for people who are moderate and high reappraisers, the effect of negative past outcome flips in strategy condition (is positive)
# for people who are low reappraisers, the effect of negative past outcome gets worse



# PLOT THESE RESULTS: 
basemodel1_stratTemporalcontextERQthirdSplit  = summary(pocShiftEarnExp_ERQreapXstratXpoc_highLowReap3rdsplit); # using model with ERQ x 3 timescales
pocBeta = basemodel1_stratTemporalcontextERQthirdSplit$coefficients[1]
stratBeta =  basemodel1_stratTemporalcontextERQthirdSplit$coefficients[2]
pocStratBeta = basemodel1_stratTemporalcontextERQthirdSplit$coefficients[8]
pocHighReapBeta = basemodel1_stratTemporalcontextERQthirdSplit$coefficients[9]
pocHighReapStratBeta = basemodel1_stratTemporalcontextERQthirdSplit$coefficients[16]
pocLowReapBeta = basemodel1_stratTemporalcontextERQthirdSplit$coefficients[11]
pocLowReapStratBeta = basemodel1_stratTemporalcontextERQthirdSplit$coefficients[17]
  


# plot results of poc x strategy x reappraisal at different levels of reappraisal -> low, moderate, high
# each figure will have two lines for strategy and reappraisal
pocVals = rep(seq(from=0, to = 1, length.out=5), times = 2)
stratVals = rep(c(-1,1), each=5);
...
# MODERATE: poc + poc x strategy (+1/-1)
pgamStratPOC_modReap = 1/(1+exp(-1*( (pocBeta*pocVals) + (pocStratBeta*pocVals*stratVals))))
 

# HIGH: poc + poc x strategy (+1/-1) + poc x high + poc x high x strategy (+1/-1)
pgamStratPOC_highReap = 1/(1+exp(-1*( (pocBeta*pocVals) + (pocStratBeta*pocVals*stratVals) + (pocHighReapBeta*pocVals) + (pocHighReapStratBeta*pocVals*stratVals) )))           

# LOW: poc + poc x strategy (+1/-1) + poc x low + poc x low x strategy (+1/-1)
pgamStratPOC_lowReap = 1/(1+exp(-1*( (pocBeta*pocVals) + (pocStratBeta*pocVals*stratVals) + (pocLowReapBeta*pocVals) + (pocLowReapStratBeta*pocVals*stratVals) )))      
                                       

# low reappraisal
pdf(file.path(config$path$directory, config$path$shlab_figures,'model_basemodel1_TERTILES_lowreap_pocStratES.pdf'))
plot(pgamStratPOC_lowReap[1:5], type="l", lwd=8, col="goldenrod1", ylab="P(gamble)", xlab="Outcome(t-1) amount", cex=2, ylim=c(.29, .61), axes=F, main="low reap: poc x strat"); # low reappraisal natural
lines(pgamStratPOC_lowReap[6:10], lwd=8, col="tomato1", lty=2); # low reappraisal, strategy
axis(1, at=c(1,3,5), label=c("$0", "$30.50", "$61"), lwd=6, cex=1.5)
axis(2, at = c(.3, .4, .5, .6), labels= c(.30, .40, .50, .60), lwd =8, cex=1.5)
abline(a=.5, b=0, col="darkgrey", lty="dotted", lwd=8)
legend("bottomleft", legend=c("Act Natural","Ignore Context"),lty=c(1,2), bty="n", col=c("goldenrod1", "tomato1"), cex=1.5, lwd=8)
dev.off()

#plot just past outcome effect in act natural for dissertation
pdf(file.path(config$path$directory, config$path$shlab_figures,'model_basemodel1_TERTILES_lowreap_pocStratES_justNatural.pdf'))
plot(pgamStratPOC_lowReap[1:5], type="l", lwd=8, col="goldenrod1", ylab="P(gamble)", xlab="Outcome(t-1) amount", cex=2, ylim=c(.29, .61), axes=F, main="low reap: poc x strat"); # low reappraisal natural
#lines(pgamStratPOC_lowReap[6:10], lwd=6, col="tomato1", lty=2); # low reappraisal, strategy
axis(1, at=c(1,3,5), label=c("$0", "$30.50", "$61"), lwd=8, cex=1.5)
axis(2, at = c(.3, .4, .5, .6), labels= c(.30, .40, .50, .60), lwd =8, cex=1.5)
abline(a=.5, b=0, col="darkgrey", lty="dotted", lwd=8)
legend("bottomleft", legend=c("Act Natural","Ignore Context"),lty=c(1,2), bty="n", col=c("goldenrod1", "tomato1"), cex=1.5, lwd=8)
dev.off()
...
pdf(file.path(config$path$directory, config$path$shlab_figures,'model_basemodel1_TERTILES_modreap_pocStratES.pdf'))
plot(pgamStratPOC_modReap[1:5], type="l", lwd=8, col="goldenrod1", ylab="P(gamble)", xlab="Outcome(t-1) amount", cex=2, ylim=c(.29, .61), axes=F, main="moderate reap: poc x strat"); # high reappraisal natural
lines(pgamStratPOC_modReap[6:10], lwd=8, col="tomato1", lty=2); # avg reappraisal, strategy
axis(1, at=c(1,3,5), label=c("$0", "$30.50", "$61"), lwd=8, cex=1.5)
axis(2, at = c(.3, .4, .5, .6), labels= c(.30, .40, .50, .60), lwd =8, cex=1.5)
abline(a=.5, b=0, col="darkgrey", lty="dotted", lwd=8)
legend("bottomleft", legend=c("Act Natural","Ignore Context"),lty=c(1,2), bty="n", col=c("goldenrod1", "tomato1"), cex=1.5, lwd=8)
...
# plot for just act natural for dissertation defense
pdf(file.path(config$path$directory, config$path$shlab_figures,'model_basemodel1_TERTILES_modreap_pocStratES_justNatural.pdf'))
plot(pgamStratPOC_modReap[1:5], type="l", lwd=8, col="goldenrod1", ylab="P(gamble)", xlab="Outcome(t-1) amount", cex=2, ylim=c(.29, .61), axes=F, main="moderate reap: poc x strat"); # high reappraisal natural
#lines(pgamStratPOC_modReap[6:10], lwd=6, col="tomato1", lty=2); # avg reappraisal, strategy
axis(1, at=c(1,3,5), label=c("$0", "$30.50", "$61"), lwd=8, cex=1.5)
axis(2, at = c(.3, .4, .5, .6), labels= c(.30, .40, .50, .60), lwd =8, cex=1.5)
abline(a=.5, b=0, col="darkgrey", lty="dotted", lwd=8)
legend("bottomleft", legend=c("Act Natural","Ignore Context"),lty=c(1,2), bty="n", col=c("goldenrod1", "tomato1"), cex=1.5, lwd=8)
...
# high reappraisal
pdf(file.path(config$path$directory, config$path$shlab_figures,'model_basemodel1_TERTILES_highreap_pocStratES.pdf'))
plot(pgamStratPOC_highReap[1:5], type="l", lwd=8, col="goldenrod1", ylab="P(gamble)", xlab="Outcome(t-1) amount", cex=2, ylim=c(.29, .61), axes=F, main="high reap: poc x strat"); # high reappraisal natural
lines(pgamStratPOC_highReap[6:10], lwd=8, col="tomato1", lty=2); # avg reappraisal, strategy
axis(1, at=c(1,3,5), label=c("$0", "$30.50", "$61"), lwd=8, cex=1.5)
axis(2, at = c(.3, .4, .5, .6), labels= c(.30, .40, .50, .60), lwd =8, cex=1.5)
abline(a=.5, b=0, col="darkgrey", lty="dotted", lwd=8)
legend("bottomleft", legend=c("Act Natural","Ignore Context"),lty=c(1,2), bty="n", col=c("goldenrod1", "tomato1"), cex=1.5, lwd=8)
...
# plot just natural for dissertation 
pdf(file.path(config$path$directory, config$path$shlab_figures,'model_basemodel1_TERTILES_highreap_pocStratES_justNatural.pdf'))
plot(pgamStratPOC_highReap[1:5], type="l", lwd=8, col="goldenrod1", ylab="P(gamble)", xlab="Outcome(t-1) amount", cex=2, ylim=c(.29, .61), axes=F, main="high reap: poc x strat"); # high reappraisal natural
#lines(pgamStratPOC_highReap[6:10], lwd=8, col="tomato1", lty=2); # avg reappraisal, strategy
axis(1, at=c(1,3,5), label=c("$0", "$30.50", "$61"), lwd=8, cex=1.5)
axis(2, at = c(.3, .4, .5, .6), labels= c(.30, .40, .50, .60), lwd =8, cex=1.5)
...
# do low reappraisers have different difficulty, frequency ratings?
lowreapdf = rcsSubLevelWide_clean[rcsSubLevelWide_clean$reapSpan0mean <=thirdSplitMEAN[1],]
modhighreapdf = rcsSubLevelWide_clean[rcsSubLevelWide_clean$reapSpan0mean >thirdSplitMEAN[1],]


lowReapDiffRatings = c(lowreapdf$round1_instDifficult[lowreapdf$round1_strategy==1],lowreapdf$round2_instDifficult[lowreapdf$round2_strategy==1]);
sd(lowReapDiffRatings, na.rm=T); #28.10081

lowReapFreqRatings = c(lowreapdf$round1_instHowOften[lowreapdf$round1_strategy==1],lowreapdf$round2_round1_instHowOften[lowreapdf$round2_strategy==1]);
sd(lowReapFreqRatings, na.rm=T); # 18.67047

highReapDiffRatings = c(modhighreapdf$round1_instDifficult[modhighreapdf$round1_strategy==1],modhighreapdf$round2_instDifficult[modhighreapdf$round2_strategy==1]);
sd(highReapDiffRatings, na.rm=T); # 24.58123

highReapFreqRatings = c(modhighreapdf$round1_instHowOften[modhighreapdf$round1_strategy==1],modhighreapdf$round2_instHowOften[modhighreapdf$round2_strategy==1]);
sd(highReapFreqRatings, na.rm=T); #20.47066

# high vs low difficluty ratings for stratey
t.test(lowReapDiffRatings, highReapDiffRatings); # welch's t-test; not significant
# t = 0.62255, df = 72.236, p-value = 0.5355
# mean low reap diff =  35.79221
# mean high reap diff = 32.56285

# high vs low frequency ratings for stratey
t.test(lowReapFreqRatings, highReapFreqRatings); # welch's t-test; not significant
# t = -0.084677, df = 38.755, p-value = 0.933
# mean low reap freq = 75.48353
# mean high reap freq = 75.86654 

# overall, no strong indication that low vs. high reapraisers had a harder time implementing the strategy overall.

# what about within groups - is there a difference in difficulty for nat v strat for low reappraisers?
lowReapDiffRatings_natural = c(lowreapdf$round1_instDifficult[lowreapdf$round1_strategy==0],lowreapdf$round2_instDifficult[lowreapdf$round2_strategy==0]);
sd(lowReapDiffRatings_natural, na.rm=T); #19.42387

t.test(lowReapDiffRatings, lowReapDiffRatings_natural); # welch's t-test;  p = .08
# mean diff ratings in strategy for low reappraisers = 35.8, SD = 28.1
# mean diff ratings in natural for high reappraisers = 26.6, SD  =19.42

lowReapFreqRatings_natural = c(lowreapdf$round1_instHowOften[lowreapdf$round1_strategy==0],lowreapdf$round2_round1_instHowOften[lowreapdf$round2_strategy==0]); 
sd(lowReapFreqRatings_natural, na.rm=T); # 15.9232

t.test(lowReapFreqRatings, lowReapFreqRatings_natural); # ns
# t = -1.2132, df = 43.256, p-value = 0.2316
# mean freq natural = 81.63656
# mean freq strategy = 75.48353

#for low repraisers, their difficulty ratings to strat vs. nat are higher but the difference is not significant.

# look at indiv. level estimtes of poc x strategy and then plot against indiv reap values
pocShiftEarnExp_pocStrategyIndiv = glmer(choice~0 + pastOC1sc*strategyRecode + signedShiftsc*strategyRecode + earnNormalizedOverall*strategyRecode + linExpectation*strategyRecode + (0 + pastOC1sc:strategyRecode|subID), data=rdmDFclean, family="binomial",offset=predModel2);
summary(pocShiftEarnExp_pocStrategyIndiv);
modelcoef = coef(pocShiftEarnExp_pocStrategyIndiv); # this would get the difference in poc across strategy


# add poc as random effect in additinoa to poc:strat ranef
pocShiftEarnExp_pocStrategyIndiv_pocRanef = glmer(choice~0 + pastOC1sc*strategyRecode + signedShiftsc*strategyRecode + earnNormalizedOverall*strategyRecode + linExpectation*strategyRecode + (0 + pastOC1sc + pastOC1sc:strategyRecode|subID), data=rdmDFclean, family="binomial",offset=predModel2);

summary(pocShiftEarnExp_pocStrategyIndiv_pocRanef);
modelcoef_pocranef = coef(pocShiftEarnExp_pocStrategyIndiv_pocRanef); 


pocStratIndiv = modelcoef$subID$`pastOC1sc:strategyRecode`
plot(rcsSubLevelWide_clean$reapSpan0mean, pocStratIndiv)

pocStratIndiv_pocRanef = modelcoef_pocranef$subID$`pastOC1sc:strategyRecode`
plot(rcsSubLevelWide_clean$reapSpan0mean, pocStratIndiv_pocRanef)

# these figures show change in past outcome effect round natural to strategy for each participant and their reappraisal score
# adding past outcome as ranef doesn't change much

```


```{r rdm-strategy-context-spanMotivationERQsupp}
# Althought ERQ appears the strongest covariate when in a model with the rest of the covariates, let's follow up with these covariates in separate models (with the caveat that we are missing 23 people when look at span data which reduced our data, slightly changes our overall results, throws off the balance of people per group, and makes model comparison difficult)
...
# for choices from people with composite span data, looking at context effects, there is no significant main effect of signed shift (p= .13), the main effect poc is trending (negative), and the main effects of earnings and expectations hold up.


# First look at motivation:
# base model 1:
pocShiftEarnExp_motivation = glm(choice~0+pastOC1sc*strategyRecode + signedShiftsc*strategyRecode + earnNormalizedOverall*strategyRecode + linExpectation*strategyRecode + strategyRecode*motivationNumeric, data=rdmDFclean, family="binomial", offset=predModel2);
summary(pocShiftEarnExp_motivation);
# AIC: 30228
# no interaction between motivation and strategy, no main effect of motivation either
# because this is not even showing up in model on its own suggests its not a strong covariate but will check with temp. context and strategy


# interact motivation with temporal context x strategy
pocShiftEarnExp_motivation3timescales = glm(choice~0+pastOC1sc*strategyRecode*motivationNumeric + signedShiftsc*strategyRecode*motivationNumeric + earnNormalizedOverall*strategyRecode*motivationNumeric + linExpectation*strategyRecode*motivationNumeric, data=rdmDFclean, family="binomial", offset=predModel2);
summary(pocShiftEarnExp_motivation3timescales);
# AIC: 30223
# motivation interacts with strategy and global timescales
# accoutning for interaction between motivation, tmp context and strat improves model


# Next look at ERQ suppression:
# base model 1:
pocShiftEarnExp_suppression = glm(choice~0+pastOC1sc*strategyRecode + signedShiftsc*strategyRecode + earnNormalizedOverall*strategyRecode + linExpectation*strategyRecode + strategyRecode*suppSpan0mean, data=rdmDFclean, family="binomial", offset=predModel2);
summary(pocShiftEarnExp_suppression);
# AIC: 29767 - missing data from two people
# no main effect of suppression and no interaction with strategy
# similar to above model with motiv, we are giving these covariates more space to explain the extra variance and there isn't a main effect or interaction with strategy

# interact suppression with strategy x temporal context
pocShiftEarnExp_suppression3timescales = glm(choice~0+pastOC1sc*strategyRecode*suppSpan0mean + signedShiftsc*strategyRecode*suppSpan0mean + earnNormalizedOverall*strategyRecode*suppSpan0mean + linExpectation*strategyRecode*suppSpan0mean, data=rdmDFclean, family="binomial", offset=predModel2);
summary(pocShiftEarnExp_suppression3timescales);
# AIC = 29768; worse AIC than above
# interaction between supp, strategy and past outcome:
  # low suppressors -> act natural: positive effect of poc; ignore context: negative effect of poc
  # avg suppressors -> similar negative effect of poc across act natural and ignore context
  # high suppressors -> act natural: negative effect of poc; ignore context: effect of poc is ~0





# Next look at composite span:

# base model 1:
pocShiftEarnExp_compositeSpan = glm(choice~0+pastOC1sc*strategyRecode + signedShiftsc*strategyRecode + earnNormalizedOverall*strategyRecode + linExpectation*strategyRecode + strategyRecode*compositeSpanScore, data=rdmDFclean, family="binomial", offset=predModel2);
summary(pocShiftEarnExp_compositeSpan);
# AIC =24272
# no main effect of span or interaction between span and strategy


# interact span with temporal context
pocShiftEarnExp_compositeSpan3timescales = glm(choice~0+pastOC1sc*strategyRecode*compositeSpanScore + signedShiftsc*strategyRecode*compositeSpanScore + earnNormalizedOverall*strategyRecode*compositeSpanScore + linExpectation*strategyRecode*compositeSpanScore, data=rdmDFclean, family="binomial", offset=predModel2);
summary(pocShiftEarnExp_compositeSpan3timescales);
# AIC: 24267 - missing a lot of data
# interaction between strategy, span and shift, earnings and expectations
# intxn with strat, span and shift:
  # low span -> act natural: negative effect of shift that is weakened in strategy
  # avg span -> act natural: negative effect of shift; ignore context: effect of shift flips to positive
  # high span -> act natural: positive effect of shift that gets stronger in strategy condition
  # across the board, regardless of span, the effect of strategy makes the shift effect more positive

# intxn with strat span and earnings:
  # low span-> act natural: neg effect of earnings; ignore context: positive effect of earnings
  # avg span -> act natural: neg effect of earnings; ignore context: positive effect of earnings
  # high span -> act natural: positive effect of earnings; ignore context: negative effect of earnings

# intxn with strat span and exp:
  # across levels of composite span scores: act natural is associated with positive effect of expectations (getting more positive as span increases) and strategy is associated with negative effect of expectations (increasingly negative as span increases)


# across these covariates (supression, motivation and span), there is no main effect of them or an interaction with strategy. With motivation and span, there is an improvement in the model if we interact them with strat x temp context. With span, we are dealing with a great loss of data and its hard to compare these models because of that. When deciding how to move forward, it makes most sense to stick with our ERQ model because that covariate was showing up from the beginning evne in a model with ALL of them in it at the same time. Will include these analyses in the supplement/appendix.


# try model comparison package:
# library(performance)
# compare_performance(pocShiftEarnExp_compositeSpan,pocShiftEarnExp_suppression,pocShiftEarnExp_motivation,pocShiftEarnExp_ERQreapXstratOnly)
# plot(compare_performance(pocShiftEarnExp_compositeSpan,pocShiftEarnExp_suppression,pocShiftEarnExp_motivation,pocShiftEarnExp_ERQreapXstratOnly, rank=T));
# # The model comparison tool is still not able to account for loss of data. It shows composite span as best overall model followed by ERQ. Still have the issue of comparing models with the loss of data.
```
...
```{r selected-outcome}
# does selected outcome from round 1 influence risk-taking?

colnames(rcsSubLevelWide_clean)
rcsSubLevelWide_clean$round1_oc_scaled = rcsSubLevelWide_clean$round1_oc/scaleby

model_rnd1oc_meanpgam= lm(round2_pgamble~round1_oc_scaled, data = rcsSubLevelWide_clean);
# no effect of round 1 outcome on round 2 pgamble (overall)


model_rnd1oc_choice = glmer(choice~ 1 + gainScaled + safeScaled + evLevScaled + roundRecode*strategyRecode + round1oc*roundRecode + (1|subID), data=rdmDFclean , family = "binomial")

summary(model_rnd1oc_choice);# no main effect of outcome from round 1 on gambling and no interaction with round

```

```{r RT-variability-fatigue}
...
pdf("/Volumes/shlab/Projects/RCS/figures/RTvariability_bySubs.pdf")
...
pdf("/Volumes/shlab/Projects/RCS/figures/meanRTvariability_acrossSubs.pdf")
par(mfrow=c(1,1))
...
# t-test to compare each bin
wilcox.test(RTvariability$bin1, RTvariability$bin2, paired=T, alternative = "two.sided"); #V = 5234, p-value = 0.0007049
wilcox.test(RTvariability$bin2, RTvariability$bin3, paired=T, alternative = "two.sided"); # V = 4543, p-value = 0.09601

wilcox.test(RTvariability$bin3, RTvariability$bin4, paired=T, alternative = "two.sided"); # V = 3632, p-value = 0.5454

wilcox.test(RTvariability$bin4, RTvariability$bin5, paired=T, alternative = "two.sided"); # V = 4294, p-value = 0.2967

wilcox.test(RTvariability$bin5, RTvariability$bin6, paired=T, alternative = "two.sided"); # V = 4011, p-value = 0.7354
