...
model_pastMeanEV_pred2 = glm(choice ~ 0 + pastMeanEV, data=rdmDFclean, family="binomial", offset=predModel2)
summary(model_pastMeanEV_pred2); # ns

model_pastChoice_pred2 = glm(choice ~ 0 + pastChoice, data=rdmDFclean, family="binomial", offset=predModel2)
summary(model_pastChoice_pred2)
# AIC: 30220 - people are more likely to gamble if they did on the previous trial - this may be capturing preferences for risk-taking (people who gamble may just gamble more often overall)
...
ERQvals = rep(seq(from = -1, to = 1, length.out = 5), times =2)
stratVals = rep(c(-1,1), each = 5)

erqstratsummary = summary(pocShiftEarnExp_ERQreapXstratOnly); # using base model 1
#erqstratsummary = summary(pocShiftEarnExp_pocIntnx_ERQreapXstratOnly); # using base model 2

# getting coefficients for model 2
# stratBeta = erqstratsummary$coefficients[3]
# erqBeta=erqstratsummary$coefficients[6]
# stratERQbeta = erqstratsummary$coefficients[13]

# coefficients from model 1
stratBeta = erqstratsummary$coefficients[2]
erqBeta=erqstratsummary$coefficients[6]
stratERQbeta = erqstratsummary$coefficients[11]

pgamStratERQ= 1/(1+exp(-1*( (stratBeta*stratVals) + (erqBeta*ERQvals) + (stratERQbeta*stratVals*ERQvals) )));
#pgamStratERQ= 1/(1+exp(-1*( (stratERQbeta*stratVals*ERQvals) )));

plot(pgamStratERQ[1:5], type="l", lwd=4, col="goldenrod1", ylab="P(gamble)", xlab="reappraisal scores", cex=2, ylim=c(.4, .6), axes=F, main="strat x ERQ reap"); # low reappraisal natural - no effect of past outcome
...
# for people with low reappraisal scores - there is no major difference of strateyg
# for people with high reappraisal scores - strategy is associated with less risk-taking
# another way - for people who are told to act natural, the effect of reappraisal score is to take more risk and for people in 
...
shiftBeta = basemodel1_erqStrat_temporalcontext$coefficients[5]
...
pocExpBeta = basemodel1_erqStrat_temporalcontext$coefficients[10]
pocEarnBeta = basemodel1_erqStrat_temporalcontext$coefficients[12]
