...
# ERQ and Demographic
ERQdemoDF = read_csv(file.path(config$path$directory, 'data/rcsERQdemo.csv'))
...
conditionsFile = read_csv(file.path(config$path$task_files, 'rdmTask/rcsConditions.csv'))
...
```{r clean-and-save-rdm, echo=FALSE}

#excludeSubID = rdmExclude$subID[(rdmExclude$round1Exclude + rdmExclude$round2Exclude)>0]
...
save(rdmDFclean, file = file.path(config$path$data$Rdata, 'rdmDF_clean.Rdata'))
...
colnames(rcsAllExclusion) = c("subID", "rdm1exclude", "rdm2exclude", "ERQexclude", "ospanExclude", "symspanExclude")

save(rcsAllExclusion, file=file.path(config$path$data$Rdata, 'rcsAllExclusion.Rdata'))
...
save(demographic, file=file.path(config$path$data$Rdata, 'demographic.Rdata'))
...
save(postTaskNumericOnly, file=file.path(config$path$data$Rdata, 'postTaskNumericOnly.Rdata'))
...
save(rdmPostRoundQsdf, file=file.path(config$path$data$Rdata, 'rdmPostRoundQs.Rdata'))
...
```{r make-subject-level-dataframes}
# want to include most things from the RDMquality data frame, ospan, symspan, ERQ, post-round qs, demographic info, outcomes selected after each round and final payment
