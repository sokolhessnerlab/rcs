...
```{r combine-files, include=FALSE, results=FALSE}
rdmDF <- file.path(rdmPath,rdmFiles) %>% 
  lapply(read_csv) %>% 
  bind_rows

rdmPostRoundQsdf <- file.path(rdmPath,rdmPostQFiles) %>% 
  lapply(read_csv) %>% 
  bind_rows

rdmOutcomes <- file.path(rdmPath,rdmOutcomeFiles) %>% 
  lapply(read_csv) %>% 
  bind_rows
...
#conditionsFile = read_csv(file.path(config$path$task_files, 'rdmTask/rcsConditions.csv')) # online
...
RDMqualityCheck = as.data.frame(matrix(data=NA, nrow=nSub, ncol=25, dimnames = list(c(NULL), c("subID", "round1missedT", "round2missedT","ShouldSafeRound1tot", "ShouldSafeRound1totMiss", "ShouldSafeRound1MissProportion", "ShouldGamRound1tot","ShouldGamRound1totMiss","ShouldGamRound1MissProportion","ShouldSafeRound2tot","ShouldSafeRound2totMiss","ShouldSafeRound2MissProportion","ShouldGamRound2tot", "ShouldGamRound2totMiss", "ShouldGamRound2MissProportion", "pgambleRound1", "pgambleRound2", "glmGainYesRound1", "glmSafeYesRound1", "glmGainYesRound2", "glmSafeYesRound2", "round1rt", "round2rt", "round1strat", "round2strat"))));
...
round1passfail$subID=1:nSub; # round 2
...
Note: Sub 37 did not complete ospan or symspan task
...
# manually exclude sub 37
symspanExclude = c(symspanExclude,"037")
ospanExclude = c(ospanExclude,"037")
...
complexSpanScores = as.data.frame(matrix(data=NA, nrow = nSub, ncol=3, dimnames=list(c(NULL), c("subID", "ospanScore", "symspanScore"))));
...
reappraisalScores = (as.numeric(ERQdemoDF$ERQ_1[3:nrow(ERQdemoDF)]) + as.numeric(ERQdemoDF$ERQ_3[3:nrow(ERQdemoDF)]) + as.numeric(ERQdemoDF$ERQ_5[3:nrow(ERQdemoDF)])+ as.numeric(ERQdemoDF$ERQ_7[3:nrow(ERQdemoDF)]) + as.numeric(ERQdemoDF$ERQ_8[3:nrow(ERQdemoDF)]) + as.numeric(ERQdemoDF$ERQ_10[3:nrow(ERQdemoDF)]))


#suppression: 2, 4, 6, 9
suppressionScores = (as.numeric(ERQdemoDF$ERQ_2[3:nrow(ERQdemoDF)]) + as.numeric(ERQdemoDF$ERQ_4[3:nrow(ERQdemoDF)]) + as.numeric(ERQdemoDF$ERQ_6[3:nrow(ERQdemoDF)]) + as.numeric(ERQdemoDF$ERQ_9[3:nrow(ERQdemoDF)]))

reapprMean = mean(reappraisalScores, na.rm=T)
suppMean =mean(suppressionScores, na.rm=T)
hist(reappraisalScores, main=sprintf("Reappraisal scores for N = %.0f \nMean = %.02f",nSub,reapprMean)) 
hist(suppressionScores, main=sprintf("Supression scores for N = %.0f \nMean = %.02f",nSub,suppMean)) 

# have not applied any exclusion at this point
ERQscores = cbind(subID,reappraisalScores,suppressionScores)
...
ERQexclude$ERQexclude[is.na(ERQscores$reappraisalScores+ERQscores$suppressionScores)] = 1
ERQexclude$ERQexclude[is.na(ERQexclude$ERQexclude)] = 0


# for offline
save(ERQscores, file = "/Users/hayley/Desktop/RCS/data/Rdata/ERQscores.Rdata")
save(ERQexclude, file = "/Users/hayley/Desktop/RCS/data/Rdata/ERQexclude.Rdata")
...
rdmDF$ERQreappraisal = rep(ERQscores$reappraisalScores, each = nT*rounds)
rdmDF$ERQsuppression = rep(ERQscores$suppressionScores, each = nT*rounds)
...
totNan = length(nanInd); # 20 missed trials after removing participants
...
rdmDFclean$ERQreappraisal[which(as.numeric(rdmDFclean$subID) %in% ERQexclude$subID[ERQexclude$ERQexclude==1])] = NA
rdmDFclean$ERQsuppression[which(as.numeric(rdmDFclean$subID) %in% ERQexclude$subID[ERQexclude$ERQexclude==1])] = NA
...
ERQscores = reappraisal and suppresion scores
...
subLevLongNames = c("subID", "pgamble", "strategy", "RT", "ospan", "symspan", "ERQreapp", "ERQsupp", "instDifficult", "instHowOften", "condCode", "rdmOverallDiff", "motivation", "rdmRoundsIndep", "gender", "genderCode", "race", "raceCode", "ethnicity", "ethnicityCode", "age", "selectedOC", "rdmOCselect", "rdmOCpayment")
...
rcsSubLevelLong$ERQreapp = rep(ERQscores$reappraisalScores, each = 2)
rcsSubLevelLong$ERQsupp = rep(ERQscores$suppressionScores, each = 2)
...
for (s in 1:nSub) {
  ...
  rcsSubLevelLong$RT[subInd[1]] = RDMqualityCheck$round1rt[s]
  rcsSubLevelLong$RT[subInd[2]] = RDMqualityCheck$round2rt[s]
  ...
  rcsSubLevelLong$selectedOC[subInd[1]] = rdmOutcomesClean$ocRound2[s]
}
...
subLevWideNames = c("subID", "round1_pgamble", "round2_pgamble","round1_strategy", "round2_strategy", "round1_RT", "round2_RT", "ospan", "symspan", "ERQreapp", "ERQsupp", "round1_instDifficult","round2_instDifficult","round1_instHowOften", "round2_instHowOften", "condCode", "rdmOverallDiff", "motivation", "rdmRoundsIndep", "gender", "genderCode", "race", "raceCode", "ethnicity", "ethnicityCode", "age", "round1_oc", "round2_oc", "rdmOCselect", "rdmOCpayment")

rcsSubLevelWide$rdmOCselect = rdmOutcomesClean$ocFinal[s]
rcsSubLevelWide$rdmOCpayment = rdmOutcomesClean$ocPayment[s]
rcsSubLevelWide$age = demographic$age[s]
rcsSubLevelWide$gender = demographic$gender[s]
rcsSubLevelWide$genderCode = demographic$genderCode[s]
rcsSubLevelWide$race = demographic$race[s]
rcsSubLevelWide$raceCode = demographic$raceCode[s]
rcsSubLevelWide$ethnicity = demographic$race[s]
rcsSubLevelWide$ethnicityCode = demographic$raceCode[s]
rcsSubLevelWide$rdmRoundsIndep = postTaskNumericOnly$roundsIndependent[s]
rcsSubLevelWide$motivation = postTaskNumericOnly$motivation[s]
rcsSubLevelWide$rdmOverallDiff = postTaskNumericOnly$rdmOverallDifficulty[s]
rcsSubLevelLong$ERQreapp = ERQscores$reappraisalScores[s]
rcsSubLevelLong$ERQsupp = ERQscores$suppressionScores[s]
rcsSubLevelLong$ospan = complexSpanScores$ospanScore[s]
rcsSubLevelLong$symspan = complexSpanScores$symspanScore[s]


#let off here, the lines above haven't been checked but should work, then the other things that need to be added might require a for lopo
...
```{r apply-and-save-clean-sub-level-dataframes}
#CREATE VERSIONS OF BOTH LONG AND WIDE BUT APPLY EXCLUSION HERE FOR RDM AND THE REST TO BOTH 
