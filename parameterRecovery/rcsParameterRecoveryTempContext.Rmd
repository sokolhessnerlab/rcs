...
iter = 1; # iterations per person (started with 700, but will lower to speed up testing)
ncores = 4; # cores we will use for parallel processing 
cIter = 10; #iterations per core --> this may not be necessary for glm...make this 100 and see what the output is. If GLM is returning slightly different values, then the fit quality is good but if its not, we may just want to make this 1 to save processing power.
```
...
#for(n in 1){ # for each person
 #for(i in 1){ # iteration per person
    ...
    indivEstimates = array(data=NA, dim = list(ncores,6,cIter)); # this will temporarily hold optimization results (pars and ll) from cores x cIter
    indivSEs = array(data=NA, dim = list(ncores,6,cIter));
    indivPvals =array(data=NA, dim = list(ncores,6,cIter));
    indivAIC = array(data=NA, dim = list(ncores,1,cIter)); 
    ...
    for(c in 1:ncores){ #in each core
      for(r in 1:cIter){ # for each iteration
        if (typeof(outputlist[[c]][[r]]) == 'list'){ #if the output is a list -- this means optim worked
          ...
          tmp= summary(outputlist[[c]][[r]]); # save this so we can get the pvalues
          indivPvals[c,1:6,r] <- tmp$coefficients[19:24]; # pull out the pvalues
          indivSEs[c,1:6,r] <-outputlist[[c]][[r]]$coefficients;
        #} else {
         # indivEstimates[c,1:6,r]<- NA ; #if optim did not work, just store as NA
         # indivEstimates[c,7,r]<- NA}
      }
    };
    
      # NEXT STEP: PULL OUT BEST AIC
...
#2/28
#roughly half (.4988) of iterations could not be computed because NaNs, half (.501) were not succesfully recovered, and .0002 (5 total) were recovered succesfully
#roughly half (.4967) of positive shift ground truths could not be computed because NaNs, half (.503) were not successfully recovered, .0001 (2 total) were recovered successfully
#roughly half of earning ground truths could not be computed because of NaNs (.4986), half were unsuccessful (.50), and 0 were successfully recovered

# Overall, terrible recovery. The majority of recovered values are hitting the bounds (looking at histograms)
