...
try:   
    ...
    #draw the stuff
        ...
        if response is None:
            ...
            response = event.waitKeys(keyList = ['v', 'n'], timeStamped = rtClock) # waiting for key press
            
            # output from response is ['n', 5.136503931000334] whre its the response key and the current time on rtClock
           
        #while rtClock.getTime()> stimTime + choiceTime:

            #response = event.waitKeys(keyList = ['v', 'n'], timeStamped = rtClock) # waiting for key press
            
    #         if (pracStart.getTime() - stimDispStart) >= stimTime:
